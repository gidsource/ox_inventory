...
-- [BARU] Event Listener untuk Toggle Blip
-- Ini akan mendeteksi blip yang ada di tabel 'shops' lokal file ini
RegisterNetEvent('global:toggleAllBlips', function(shouldShow)
    for i = 1, #shops do
        local shop = shops[i]
        -- Cek apakah toko ini memiliki blip dan blip tersebut valid
        if shop and shop.blip and DoesBlipExist(shop.blip) then
            if shouldShow then
                SetBlipDisplay(shop.blip, 4) -- Tampilkan (Map & Minimap)
            else
                SetBlipDisplay(shop.blip, 0) -- Sembunyikan
            end
        end
    end
end)
